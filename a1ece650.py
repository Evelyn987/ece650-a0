        ...
        if entry != 'g':
            ...
            for k in list_v4:
                if not k in list_eagle:
                    list_eagle.append(k)
            # print list_eagle
            # print len(list_eagle)
            # print list_v6
            # print len(list_v6)

            for w in range(len(list_eagle) - 1, -1, -1):
                for z in range(len(list_v6) - 1, -1, -1):
                    if (min(list_eagle[w][0][0], list_eagle[w][1][0]) < list_v6[z][0] < max(list_eagle[w][0][0],
                                                                                            list_eagle[w][1][0])) or (
                            min(list_eagle[w][0][1], list_eagle[w][1][1]) < list_v6[z][1] < max(list_eagle[w][0][1],
                                                                                                list_eagle[w][1][1])):
                        del list_eagle[w]
                ...
                for j in range(i + 1, len(list_v6)):
                    for v in range(len(list_v2)):
                        if (min(list_v6[i][0], list_v6[j][0]) < list_v2[v][0] < max(list_v6[i][0], list_v6[j][0])) or (
                                min(list_v6[i][1], list_v6[j][1]) < list_v2[v][1] < max(list_v6[i][1],
                                                                                        list_v6[j][1])) == 0:
